...
import { CENTER_ANIMATION_DURATION_MS } from './constants';
...
function getGroupByFieldOptionsFromDataView(
  dataView:
    | { fields: Array<{ name: string; aggregatable?: boolean }> | { getAll(): Array<{ name: string; aggregatable?: boolean }> } }
    | undefined
  ...
  const aggregatable = all.filter(
    (f: { name: string; aggregatable?: boolean }) =>
      ...
      !EXCLUDED_FIELD_PREFIXES.some((p) => f.name.startsWith(p))
  );
    ...
    >
      <div css={panelStyle}>
        ...
        <EuiSwitch
          ...
          onChange={(e) =>
            onControlStateChange({ showOnlyActiveAlerts: e.target.checked })
          }
        ...
        <EuiFlexGroup direction="column" gutterSize="s">
          <EuiFlexItem grow={false}>
            <EuiSwitch
              ...
              onChange={(e) =>
                onControlStateChange({ showSloBadge: e.target.checked })
              }
          ...
          <EuiFlexItem grow={false}>
            <EuiSwitch
              ...
              onChange={(e) =>
                onControlStateChange({ showAlertsBadge: e.target.checked })
              }
          ...
          <EuiFlexItem grow={false}>
            <EuiSwitch
              ...
              onChange={(e) =>
                onControlStateChange({ showAnomalyBadge: e.target.checked })
              }
        ...
        <EuiComboBox
          ...
          selectedOptions={
            controlState.groupBy
              ? [{ label: controlState.groupBy, value: controlState.groupBy }]
              : []
          }
          onChange={(selected) => {
            const next =
              selected.length === 0 ? null : (selected[0].value ?? selected[0].label);
        ...
        <EuiFlexGroup gutterSize="s">
          ...
          <EuiFlexItem grow={false}>
            <EuiButtonEmpty
              ...
              iconType="arrowDown"
              onClick={() =>
                onControlStateChange({ layoutDirection: 'vertical' as LayoutDirection })
              }
